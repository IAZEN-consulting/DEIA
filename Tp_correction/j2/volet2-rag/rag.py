...
# ///
"""Corrige du TP Jour 2, volet 2 : indexation, chaine RAG, comparaison.

    uv run rag.py indexer               etape 1 : decoupe et indexe le corpus
    uv run rag.py demander              etape 2 : interroge avec RAG
    uv run rag.py comparer              etape 3 : sans RAG puis avec RAG
    uv run rag.py demander "..." "..."   les questions du TP, ou les tiennes
...
Jour 2. OpenRouter expose aussi /embeddings, donc l'indexation et la chaine
passent par la meme cle : plus besoin d'OPENAI_API_KEY pour ce volet.
...
BASE = Path(__file__).parent / "chroma_db"
...
def cle():
    ...
    if not valeur:
        raise SystemExit("OPENROUTER_API_KEY absente : la definir dans %s" % ENV)
    return valeur


def embeddings():
    """check_embedding_ctx_length=False envoie le texte brut.

    Par defaut langchain_openai tokenise avec tiktoken et poste des tableaux
    d'identifiants. L'endpoint d'OpenAI l'accepte, mais les autres fournisseurs
    d'OpenRouter repondent 422 "Input should be a valid string".
    """
    from langchain_openai import OpenAIEmbeddings

    return OpenAIEmbeddings(model=EMBEDDING, base_url=BASE_URL, api_key=cle(),
...
def retrouveur(mode):
    ...
    if not BASE.exists():
        sys.exit("Base absente. Lancer d'abord : uv run rag.py indexer")
...
def chaine(mode):
    ...
    llm = ChatOpenAI(model=MODELE, temperature=0, base_url=BASE_URL, api_key=cle())
